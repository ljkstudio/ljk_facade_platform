...
| 6~12. Core 포팅 (`MoldResult`·`Grid`·`Validation`·`Optimization`·`Extension`·`Projection`·`MoldSolver`) | **✅ 완료** — 97/97, T8 중앙값 18.6 ms |
| 13. 저널·완료 판정 | **✅ 완료** — `J-016` |
| **M2b 전체** | **✅ 완료 (2026-08-15)** — 다음은 M3 (GH 어댑터 배선) |
...
## Task 6: `MoldResult` · `MoldMessage` — 출력 그릇 — ✅ 완료 (2026-08-15)
...
## Task 7: `Grid` — 픽스처가 직접 덮는 첫 모듈 — ✅ 완료 (2026-08-15)
...
## Task 8: `Validation` — 입력 정규화와 지오메트리 추출 — ✅ 완료 (2026-08-15)
...
- [x] **Step 2: 실패 확인** — `dotnet build` 에서 `Validation` 없음
...
## Task 9: `Optimization` — Phase B — ✅ 완료 (2026-08-15)
...
- [x] **Step 2: 실패 확인** — `Optimization` 없음
...
## Task 10: `Extension` — Phase C — ✅ 완료 (2026-08-15)
...
## Task 11: `Projection` — Phase D, 배열 5개를 직접 대조 — ✅ 완료 (2026-08-15)
...
- [x] **Step 1: 테스트를 먼저 쓴다 — 여기가 완료조건 3·4다**
...
## Task 12: `MoldSolver` — 계약과 전체 파이프라인 — ✅ 완료 (2026-08-15)
...
- [x] **Step 4: 통과 확인 — 전 케이스 end-to-end**
...
- [x] **Step 3: 완료 조건을 확인한다**

스펙 §8 기준:

- [x] 3. 골든 픽스처 T1~T9 를 허용오차 안에서 재현하고 **플래그는 정확히 일치**
- [x] 4. **폴백 가지가 핀별로 일치** — 커버된 가지에 한해. 커버리지 숫자를 저널에 명시
- [x] 5. T8 중앙값 ≤ 500 ms, `info` 가 경과 ms 보고 *(경과 ms 는 M3 에서 `info` 에 넣는다 — 여기서는 측정만)*

- [x] **Step 4: 스펙의 M2b 행을 완료로 바꾸고 커밋**
