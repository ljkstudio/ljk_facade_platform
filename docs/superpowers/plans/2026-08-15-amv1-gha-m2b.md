...
## Task 2: 폴백 가지 진단을 Phase B·C 로 넓힌다
...
- [ ] **Step 1: `optimization.py` 상단에 가지 상수를 넣는다**
...
- [ ] **Step 2: `optimize_surface` 가 가지를 함께 돌려주게 한다**
...
- [ ] **Step 4: `adaptive_mold_v1.py` 에서 배선한다**
...
- [ ] **Step 5: `dump_fixtures.py` 가 두 값을 덤프하게 한다**
...
## Task 3: 중간 산출물 지문 — Phase B·C 를 직접 대조 가능하게 만든다
...
- [ ] **Step 1: `run_case` 가 중간 산출물의 지문을 재게 한다**
...
- [ ] **Step 2: `expected` 에 넣고 `compare()` 에도 넣는다**
...
- [ ] **Step 4: 기존 6배열 불변을 확인하고 기준을 갱신한다**
...
- [ ] **Step 5: 지문이 케이스마다 실제로 다른지 본다 — 같으면 쓸모가 없다**
...
- [ ] **Step 1: 메시지를 수집할 수 있게 가짜 component 를 만든다**
...
- [ ] **Step 3: 지금 나오는 메시지를 먼저 본다 — 고치기 전에**
...
- [ ] **Step 5: 기존 메시지를 한국어로 통일하고 규약에 맞춘다**
...
- [ ] **Step 6: 재생성하고 불변을 확인한다 — 여기가 이 태스크의 합격 조건**
...
- [ ] **Step 7: 클램핑 경고가 실제로 T3 에 뜨는지 확인한다**
