...
**운영 함정 셋:**
1. **`yak cache` 는 자동 갱신되지 않는다.** 인덱스가 있으면 그것이 정본이므로 **낡은 인덱스가 새 패키지를 감춘다.** 배포 폴더에 `.yak` 을 올릴 때마다 재실행을 **배포 스크립트에 넣는다.** **단 `yak cache` 는 인자를 받지 않는다**(실측, J-015 FACT-03) — Rhino 에 등록된 소스 전체를 대상으로 돌므로 **소스 등록이 선행 조건**이다.
...
> **해소됨 (2026-08-15, J-015 FACT-01).** "로컬 소스에서는 이름·버전 외 필드가 전부 빈칸"이라는 보고는 **이 환경에서 재현되지 않았다** — 패키지 매니저 UI 에 `authors: LJK STUDIO` 가 정상 표시됐다. 보고 자체가 틀렸는지 특정 버전 한정인지는 모르지만, 우리 환경에서는 문제가 없다.
...
| ~~**M2b**~~ | ~~파이썬 선반영(신규 메시지 4 · `optBranch`/`extBranch` · T9) → 픽스처 재생성 → **`expected` 6배열 불변 확인** → Core 포팅 A→D→B→C~~ | **완료 (2026-08-15, J-016).** 97/97 통과, 9케이스 값·플래그·가지·메시지 일치, T8 중앙값 18.6 ms. 포팅 순서는 **A→B→C→D** 로 바꿨다(중간 지문이 생겨 B·C 를 독립 판정할 수 있게 됐다 — J-016 DECISION-01). 남은 것: 미커버 가지 3 (`ray-/ext`·`closest`·`default`) |
...
3. 골든 픽스처 T1~T9 를 허용오차 안에서 재현하고, **플래그는 정확히 일치**한다 — **✅ Core 기준 충족 (M2b, J-016 FACT-05).** 허용오차는 조정하지 않았다(`1e-6`). 어댑터를 거친 재확인은 M3
4. **폴백 가지 선택이 파이썬과 핀별로 일치**한다 — *커버된 가지에 한해* (§5.1) — **✅ 충족, 커버리지 4/7** (J-016 FACT-01). 미커버 `ray-/ext`·`closest`·`default`. Phase B·C 는 각각 1/7·1/3 (FACT-02)
5. **T8 중앙값 ≤ 500 ms** — 워밍업 1회 + 측정 5회 이상, **중앙값**(평균·최솟값 아님). 그리고 `info` 가 경과 ms 를 보고한다 — **측정 부분 ✅ 18.6 ms** (J-016 FACT-04). `info` 의 경과 ms 보고는 M3
