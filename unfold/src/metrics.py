...
# σ 가 1 을 이만큼 넘어야 주름 위험으로 센다.
#
# **이 값은 물리가 아니라 수치 잔차 기준이다.** 실측 2026-08-14: Rhino 로
# 메쉬화한 원통(정확한 가전개면)에서 요소 3120/6281 개가 이 문턱을 넘었는데
# 실제 압축량은 0.001% 수준이었다. 그래서 개수만으로 판단하면 안 되고
# **경고 문구가 크기를 함께 말해야 한다** — 아래 evaluate() 의 문구가 그 역할이다.
# 문턱을 물리적 크기(예: 0.1%)로 올리는 것은 v1 에서 하지 않는다. 판재·두께마다
# 다른 값을 근거 없이 못 박게 되기 때문이다(material.py 의 '가짜 손잡이' 문제).
WRINKLE_TOL = 1e-6
...
def evaluate(res, props):
    ...
    if not counted:
        ...
        if wrinkle:
            checks.append(("주름", "경고",
                           "성형 중 압축되는 요소 %d개 (전체 %d개). 최대 압축 %.3f%% "
                           "(σ = %.6f) — 판재는 압축을 받으면 주름진다"
                           % (len(wrinkle), len(res.sigmas),
                              -forming_strain(s_max) * 100.0, s_max)))
        else:
            checks.append(("주름", "통과",
                           "모든 요소가 σ ≤ 1 이다 (최대 %.6f) — 성형이 인장만으로 이루어진다"
                           % s_max))
